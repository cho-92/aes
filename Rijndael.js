...
function Rijndael(){
	...
	this.rounds=[[	10,	12,	14],//	16
				[	12,	12,	14],//	24
				[	14,	14,	14]];//	32
	
	this.expandedKeys={};//object to keep keys we've already expanded in.
	...
	this.ExpandKey=function(key) {
	  if(!this.expandedKeys[key]){
		  ...
		  for(var i = kl; i < ks; i += 4) {
			...
			if (i % kl == 0) {
			  temp = [	this.Sbox[temp[1]] ^ Rcon,	this.Sbox[temp[2]], 
						this.Sbox[temp[3]], 			this.Sbox[temp[0]]]; 
			  if ((Rcon <<= 1) >= 256)
				Rcon ^= 0x11b;
			}
			else if ((kl > 24) && (i % kl == 16))
			  temp = [this.Sbox[temp[0]], this.Sbox[temp[1]], 
	...
	this.crypt=function(block, key,encrypt) {
	  var bB=block.length;
	  var kB = key.length;
	  var bBi=0;
	  var kBi=0;
	...
	this.Sbox = new Array(99,124,119,123,242,107,111,197,48,1,103,43,254,215,171,
	...
	this.rowshifts=[[0,	1,	2,	3],		//16
				   [0,	1,	2,	3],		//24
				   [0,	1,	3,	4]];	//32
	
	this.ShiftRowTab = Array(3);
...
//public
Rijndael.prototype.Encrypt=function(block, key) {
	this.crypt(block,key,true);
};
Rijndael.prototype.Decrypt=function(block, key) {
